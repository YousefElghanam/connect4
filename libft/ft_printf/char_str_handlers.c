...
/*   Updated: 2025/03/14 02:46:16 by bgebetsb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_string(t_stringlist **string, t_properties *prop, va_list *args)
{
	char	*str;
	char	*newstr;
	int		length;
	...
	if (newstr != NULL)
	{
		...
		*string = stringlist_append(*string, newstr, ft_strlen(newstr));
	}
	else
		stringlist_clear(string);
}
		...
		va_list *args)
{
	...
	if (str != NULL)
	{
		if (properties->flags[1])
			...
			str[width - 1] = (char)c;
		*string = stringlist_append(*string, str, width);
	}
}
...
void	handle_percent(t_stringlist **string)
{
	...
	{
		str[0] = '%';
		*string = stringlist_append(*string, str, 1);
	}
}
