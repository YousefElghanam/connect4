...
/*   Updated: 2025/03/14 02:46:41 by bgebetsb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	count_length(unsigned long long n, t_properties *properties,
		t_chars c)
{
	int		length;
...
static void	fill_recursive(unsigned long long n, char *nb_str, t_chars c,
		int remaining)
{
	size_t	base;
...
char	*ft_itoa_base(unsigned long long n, t_properties *properties, t_chars c)
{
	int		length;
	ssize_t	allocated;
	char	*nb_str;
	...
	if (allocated > length)
		ft_memset(nb_str + (properties->flags[1] * length), ' ', allocated
			- length);
	return (nb_str);
}
